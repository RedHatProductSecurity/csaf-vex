...
class CSAFVEXBuilder:
    ...
    def _create_relationship(component: str, stream: str) -> Relationship:
        ...
        full_product = FullProductName(
            name=f"{component} as of component of {stream}",
    ...
    def _build_vulnerability(
        ...
        if data.get("score") and data["score"].get("vector"):
            product_ids = []
            if product_tree_data and product_tree_data.get("products"):
                product_ids = [
        ...
        if data.get("remediation"):
            vuln_dict["remediations"] = [{"category": "mitigation", "details": data["remediation"]}]
